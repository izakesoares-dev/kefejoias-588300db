...
export const CartProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addItem = (product: Product, quantity = 1, size?: number) => {
    setItems((prev) => {
  ...
  return (
    <CartContext.Provider
      value={{ items, addItem, removeItem, updateQuantity, clearCart, totalItems, subtotal, isCartOpen, setIsCartOpen }}
    >
      {children}
    </CartContext.Provider>
  );
